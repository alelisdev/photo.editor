...
var CanvasFreeDrawing = /** @class */ (function () {
    function CanvasFreeDrawing(params) {
        ...
        var cursorElement = params.cursorElement;
        ...
        this.cursorElement = cursorElement ? cursorElement : false;
    ...
    CanvasFreeDrawing.prototype.mouseDown = function (event) {
        if (event.button !== 0)
            return;
        if (!this.lazyBrush) this.newLazyBrush(event);

        this.lazyBrush.update({ x: event.offsetX, y: event.offsetY });
        var coords = this.lazyBrush.getBrushCoordinates();
    ...
    CanvasFreeDrawing.prototype.mouseMove = function (event) {
      ...
      x -= this.cursorElement.offsetWidth / 2;
      y -= this.cursorElement.offsetHeight / 2;

      this.cursorElement.style.left = Math.floor(x + event.offsetX * this.cssTransformScale) + "px";
      this.cursorElement.style.top = Math.floor(y + event.offsetY * this.cssTransformScale) + "px"; */
    ...
    CanvasFreeDrawing.prototype.touchStart = function (event) {
        if (event.changedTouches.length > 0) {
          ...
          /*
            ...
            x /= (this.cssTransformScale ? this.cssTransformScale : 1);
            y /= (this.cssTransformScale ? this.cssTransformScale : 1);

            /*var scaleOffsetX = this.cssTransformOriginalImageWidth * (this.cssTransformScale - this.cssTransformNativeScale) / this.cssTransformScale / 2;
            var scaleOffsetY = this.cssTransformOriginalImageHeight * (this.cssTransformScale - this.cssTransformNativeScale) / this.cssTransformScale / 2;

            var zoomOffsetLeftOriginX = this.cssTransformOffsetLeftOriginX / this.cssTransformScale * -1;
            var zoomOffsetLeftOriginY = this.cssTransformOffsetLeftOriginY / this.cssTransformScale * -1;

            var zoomOffsetX = this.cssTransformOffsetX / this.cssTransformScale * -1;
            var zoomOffsetY = this.cssTransformOffsetY / this.cssTransformScale * -1;

            if (this.cssTransformNativeScale > this.cssTransformScale) {
              scaleOffsetX = scaleOffsetY = zoomOffsetLeftOriginX = zoomOffsetLeftOriginY = zoomOffsetX = zoomOffsetY = 0;
            }

            x += scaleOffsetX + zoomOffsetLeftOriginX + zoomOffsetX;
            y += scaleOffsetY + zoomOffsetLeftOriginY + zoomOffsetY; */
    ...
    CanvasFreeDrawing.prototype.enableEraseMode = function () {
      this.context.globalCompositeOperation = "destination-out";
    };
    CanvasFreeDrawing.prototype.disableEraseMode = function () {
    ...
    CanvasFreeDrawing.prototype.toggleCursor = function () {
        this.canvas.style.cursor = this.isDrawingModeEnabled ? 'none' : 'auto';
    };
    ...
    CanvasFreeDrawing.prototype.newLazyBrush = function (event) {
      this.lazyBrush = new LazyBrush({
        radius: 0,
        enabled: true,
        initialPoint: { x: event.offsetX, y: event.offsetY }
      });
